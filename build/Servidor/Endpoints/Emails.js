...
router.post('/email-confirmacao', async (Pedido, Resposta) => {
    ...
    if (!emailUtilizador) {
        ...
        const [Existe] = await Globais_1.DB.execute('SELECT email FROM utilizadores WHERE email = ?', [Email]);
        if (Existe.length > 0) {
    ...
    if (emailUtilizador) {
        ...
        Globais_1.ServicoEmail.sendMail(OpcoesEmail, (Erro, Info) => {
            {
...
router.post('/email-contacto', async (Pedido, Resposta) => {
    try {
        ...
        // const Email = Pedido.body.email
        const [rows] = await Globais_1.DB.execute('SELECT id_utilizador, nome, nif FROM utilizadores WHERE email = ?', [email]);

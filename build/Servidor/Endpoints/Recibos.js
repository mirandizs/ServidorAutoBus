...
router.post('/comprar', async (Pedido, Resposta) => {
    ...
    if (informacoesPedido.guardarCartao) {
        const [GuardarCartao] = await Globais_1.DB.execute(queryGuardarCartao, [
    ...
    `;
    const [Carrinho] = await Globais_1.DB.execute(query, [idUtilizador]); // executa a query com os valores passados. tambem pode usar "as any[]" para evitar erros do typescript
    ...
    Carrinho.forEach((Bilhete) => {
        queryCompras += `(?, ?, ?, ?, ?, ?, ?),`;
        ValoresPassados.push(idUtilizador, Bilhete.id_ponto_partida, Bilhete.id_ponto_chegada, Bilhete.preco, Bilhete.data, Bilhete.hora, Bilhete.tipo);
    });
    queryCompras = queryCompras.slice(0, -1); // remove a última vírgula para nao dar erro de sintaxe de SQL
    const [Resultado] = await Globais_1.DB.execute(queryCompras, ValoresPassados); // executa a query com os valores passados. 
    ...
    `;
    await Globais_1.DB.execute(queryRemoverCarrinho, [idUtilizador]); // executa a query com os valores passados.
    Resposta.send();
});
...
router.get('/recibo/:id', async (Pedido, Resposta) => {
    ...
    const [result] = await Globais_1.DB.execute(query, [idUtilizador, idCompra]);
    ...
    const qrDataURL = await qrcode_1.default.toDataURL(conteudoQR);
